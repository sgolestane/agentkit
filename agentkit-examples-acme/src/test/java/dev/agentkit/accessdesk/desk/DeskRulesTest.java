...
class DeskRulesTest {
    ...
    private DeclaredTools as(String who) {
        return new DeskTools(who, ledger, company, clock::get).catalog();
    }
    ...
    @Test
    void aGrantIsASubjectWhoseRecordBoundsTheWorkScheduledAboutIt() {
        call(as(PRIYA), "submit_access_request", "resource_id", "db-payments-prod", "level", "read", "hours", 4,
                "justification", "INC-4211", "approver_email", DANA);
        ...
        assertThat(grant.isContact(SAM)).isFalse();
        assertThat(grant.facts()).containsEntry("expires_at", "2026-09-16T17:00:00Z").containsEntry("status", "ACTIVE");
        assertThat(DeskTools.holdings(grant)).containsExactly("GR-1001");

        call(as(PRIYA), "revoke_grant", "grant_id", "GR-1001", "reason", "done early");
        SubjectRecord revoked = DeskTools.grantSubjects(ledger).resolve(DeskTools.GRANT, "GR-1001").orElseThrow();
        assertThat(revoked.facts()).containsEntry("status", "REVOKED");
        assertThat(DeskTools.holdings(revoked)).isEmpty();
    }
}
